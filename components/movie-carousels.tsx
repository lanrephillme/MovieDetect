...
export function MovieCarousels() {
  const [carousels, setCarousels] = useState<Record<string, CarouselData>>({
    similar: { title: "Similar Movies", movies: [], loading: true, error: null },
  ...
  useEffect(() => {
    ...
    setCarousels((prev) => ({
      ...
      similar: {
        ...prev.similar,
        movies: generateMockMovies(6, 100),
        loading: false,
      },
    }))
    ...
    setCarousels((prev) => ({
      ...
      recommendations: {
        ...
        movies: generateMockMovies(6, 200).map((movie) => ({
          ...movie,
    ...
    setCarousels((prev) => ({
      ...
      watchlist: {
        ...prev.watchlist,
        movies: generateMockMovies(4, 300),
        loading: false,
      },
    }))
  }, [])
  ...
  return (
    <>
      <div className="space-y-8 py-12 bg-black">
        {Object.entries(carousels).map(([key, data]) => renderCarousel(key, data))}
      </div>
